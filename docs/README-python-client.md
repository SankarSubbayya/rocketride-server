...
**Same MIME for webhooks and `/task/data`:** You can use `application/rocketride-question` with any path that opens a data pipe (including HTTP POST to the webhook). The engine validates the body with the Pydantic `Question` model, same as chat.

**Plain `text/*` and the questions lane:** If only the **`questions`** lane is wired, the server routes `text/*` to that lane and wraps the text in a `Question`. If only **`text`** is wired, `text/*` uses the text lane. If **both** are wired, the server sends the same string to **both** lanes (`writeText` and `writeQuestions`) in one upload.
