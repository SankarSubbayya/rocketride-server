...
**Webhooks and `/task/data`:** The same MIME type and serialized `Question` body work for HTTP POST to the webhook (or task data endpoint), not only chat UI flows.

**Plain `text/*`:** If only **`questions`** is connected, `text/*` is wrapped as a `Question`. If only **`text`** is connected, `text/*` stays on the text lane. If **both** are connected, the same payload is delivered to **both** lanes in one upload.
